function [H1, H2, S1, S2, residual] = NetNMF(A1, A2, A12, parameters)
...
while ((res > tol) && (iter < max_iter))
   ...
   numerator = 2 * A1 * (H1 * S1) + lamda1 * (A12 * H2);
   denominator = 2 * H1 * (S1 * (H1' * H1) * S1) + lamda1 * H1 * (H2' * H2) + eps;
   H1 = H1 .* (numerator ./ denominator);
   clear denominator numerator
   
   % step 2b: Fix H1, S1, S2, update H2
   numerator = 2 * lamda2 * A2 * (H2 * S2) + lamda1 * (A12' * H1);
   denominator = 2 * lamda2 * H2 * (S2 * (H2' * H2) * S2) + lamda1 * H2 * (H1' * H1) + eps;
